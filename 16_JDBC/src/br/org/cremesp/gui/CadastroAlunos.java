...
public class CadastroAlunos extends JFrame {
	 ...
	 * @throws Exception 
	 */
	public CadastroAlunos() throws Exception {
		...
		btnIncluirAluno.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					RepositorioDao.getAlunosDao().incluir((Aluno) pessoa);
					listarAlunos();
...
//		});
		...
		btnIncluirCurso.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					Curso curso = new Curso(codigo, descricao, ch, preco, aluno);

					RepositorioDao.getCursosDao().incluir(curso);
		...
		btnIncluirCurso.setBounds(234, 80, 95, 23);
		panelCurso.add(btnIncluirCurso);
		
		cmbAlunos = new JComboBox<Aluno>();
		cmbAlunos.setBounds(66, 76, 155, 22);
		panelCurso.add(cmbAlunos);
		
		JLabel lblNewLabel_5_4 = new JLabel("Alunos:");
		lblNewLabel_5_4.setVerticalAlignment(SwingConstants.TOP);
		lblNewLabel_5_4.setBounds(10, 80, 46, 14);
		panelCurso.add(lblNewLabel_5_4);
		...
		lblNewLabel_5_5.setBounds(10, 38, 46, 14);
		panelMatricula.add(lblNewLabel_5_5);

		cmbCursos = new JComboBox<>();
		cmbCursos.setBounds(66, 34, 155, 22);
...
//		incluirCursos();
	...
	private void listarAlunos() {
		try {
			cmbAlunos.removeAll();
			List<Aluno> alunos = RepositorioDao.getAlunosDao().listar();
			alunos.forEach(s -> cmbAlunos.addItem(s));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	
	private void listarCursos(int idAluno) {
		try {
			List<Curso> cursos = RepositorioDao.getCursosDao().listarPorAluno(idAluno);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
