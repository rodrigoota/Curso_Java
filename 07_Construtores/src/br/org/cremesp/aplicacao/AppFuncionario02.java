...
public class AppFuncionario02 {
	...
	private static Empresa getEmpresa() throws ClassNotFoundException, ParseException {
		...
		String razsoc = showInputDialog("Informe a razão social da empresa:");
		
		DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
		Date data = df.parse(showInputDialog("Informe a data de fundação (dd/MM/aaaa)"));
		
		double area = Double.parseDouble(showInputDialog("Informe a área útil da empresa:"));
		...
		if(opcao == YES_OPTION) {
			empresa = new Empresa(razsoc, area, data, getEndereco());
			//empresa.setEndereco();			
		} else {
			empresa = new Empresa(razsoc, area, data);
		}
